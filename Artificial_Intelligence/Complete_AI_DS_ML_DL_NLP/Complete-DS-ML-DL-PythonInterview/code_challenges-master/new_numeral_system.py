...
def new_numeral_system(number):
    ...
    dict_ = {k: v for k, v in enumerate(letters)}
    ans = []
    letter_1_index = 0
    letter_2_index = letters.index(number)

    while letter_2_index >= letter_1_index:
        temp = dict_[letter_1_index] + ' + ' + dict_[letter_2_index]
        ans.append(temp)
        letter_1_index += 1
        letter_2_index -= 1
