...
def forward_propagation(X, parameters, activation_fn):
    ...
    Arguments:
    X -- input dataset_D placeholder, of shape (number of examples, input size).
...
def model(X_train, Y_train, X_test, Y_test, layers_dims, learning_rate=0.0001,
    ...
    X_test -- tensorbroad_pb_android input data, shape: num of tensorbroad_pb_android examples x num pf features.
    Y_test -- tensorbroad_pb_android label vector, shape: num of tensorbroad_pb_android examples x output size.
    ...
    with tf.Session(graph=g) as sess:
        ...
        # Print out the accuracy rates of training and tensorbroad_pb_android sets
        print("The training accuracy rate is: {:.2f}%".format(
            accuracy.eval(feed_dict={X: X_train, Y: Y_train}) * 100))
        print("The tensorbroad_pb_android accuracy rate is: {:.2f}%".format(
